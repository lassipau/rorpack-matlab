...
The \texttt{rorpack} Matlab library can be installed for Matlab R2019a and later\footnote{Earlier versions should be fine, but have not been tested. Let us know if issues arise!} as instructed in the \texttt{README.md} file included in the software package (this mainly involves copying the package on your hard drive and adding the ``RORPack'' folder and its subfolders to the Matlab PATH). The subdirectory \texttt{examples/} contains the included PDE examples and simulation files (documented in detail in Section~\ref{sec:PDEcases}).
