...
\subsubsection*{Case 3: Viscous damping, boundary input and pointwise measurement}
~\\[-1ex]

  \noindent Main file name: \texttt{Wave1DCase3\_Main.m}

  \medskip

The model on $\Omega = [0,1]$ is
\eq{
  x_{tt}(\xi,t) &=  x_{\xi\xi}(\xi,t) - d(\xi)x_t(\xi,t)\\
      -x_\xi(0,t) &= u(t) + \wdist(t), \qquad 
      x(1,t) = 0 \\
x(\xi,0)&=x_0(\xi), \quad x_t(\xi,0)=x_1(\xi)\\
      y(t) &= \pmat{x(\xi_0,t)\\x_t(\xi_0,t)} ,
    }
where $\xi_0\in\Omega$ and $d$ is a continuous function representing the location and strength of the viscous damping.
The uncontrolled system is exponentially stable due to the viscous damping. 
%Since the input and output are distributed, it is also not exponentially stabilizable or detectable, but instead it is only strongly (or polynomially~\cite{BorTom10}) stabilizable provided that $\iprod{b(\cdot)}{\sin(k\pi \cdot)}_{\Lp[2]}\neq 0$ for all $k\in\N$.
In the output regulation problem, we choose one of the outputs to be the regulated output (the choice can be made using the variable \texttt{p\_ind}.
It is possible to use the Low-Gain Robust Controller, or one of the two observer based controller designs. 
With the choice $\xi_0=0$ and the second output, the system is also impedance passive, and it's possible to use the 
 Passive Robust Controller (\texttt{PassiveRC})
